...
class TimersController extends AppController {
	...
	*/
	function view($id = null) {
		...
		if($timer['Timer']['user_id']!=$this->Session->read('Auth.User._id') && $this->Session->read('Auth.User.group_id') != '1'){
			$this->Session->setFlash('Invalid timer', 'default', array('class' => 'flash_bad'));
			$this->redirect(array('action' => 'index'));
		}
	}
	...
	function add() {
		if ($this->RequestHandler->isXml()){
			...
			if(isset($this->params['url']['title'],$this->params['url']['description'],$this->params['url']['projectId'],$this->params['url']['projectName'],$this->params['url']['time'])){
				$this->data['Timer']['user_id'] = $this->Session->read('Auth.User._id');
				...
				if($this->Timer->save($this->data)){
					$indexData = array('text'=>$this->data['Timer']['title'],'title'=>$this->data['Timer']['title'],'description'=>$this->data['Timer']['description'],'projectName'=>$this->data['Timer']['project_name'],'user_id'=>$this->Session->read('Auth.User._id'));
					$id = $this->Timer->id;
	...
	*/
	function edit($id = null) {
		...
		if (empty($this->data)) {
			...
			//check timer belongs to user or admin
			if($this->data['Timer']['user_id']!=$this->Session->read('Auth.User._id') && $this->Session->read('Auth.User.group_id') != '1'){
				$this->Session->setFlash('Invalid timer', 'default', array('class' => 'flash_bad'));
				$this->redirect(array('action' => 'index'));
			}
		}
	...
	*/
	function delete($id = null) {
		...
		if($userTimer['Timer']['user_id']!=$this->Session->read('Auth.User._id') && $this->Session->read('Auth.User.group_id') != '1'){
			$this->Session->setFlash('Invalid timer', 'default', array('class' => 'flash_bad'));
			$this->redirect(array('action' => 'index'));
		}
	...
	function search(){
		...
		foreach($res->results as $doc_id){
			$params = array(
				'conditions' => array('_id' => $doc_id->docid,'user_id'=>$this->Session->read('Auth.User._id'))
			);
