 ...
 */
class MyCacheHelper extends AppHelper {
 ...
 */
	function cache($file, $out, $cache = false) {
		$cacheTime = 0;
		$useCallbacks = false;
		if (is_array($this->MycacheAction)) {
			$keys = array_keys($this->MycacheAction);
			$index = null;
			...
			$options = $this->MycacheAction;
			if (isset($this->MycacheAction[$index])) {
				if (is_array($this->MycacheAction[$index])) {
					$options = array_merge(array('duration' => 0, 'callbacks' => false), $this->MycacheAction[$index]);
				} else {
					$cacheTime = $this->MycacheAction[$index];
				}
			}
